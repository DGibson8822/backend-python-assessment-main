...
from ..routers import breeds, pets, health
...
# Merged both service files into one as the logic identical, though I could
# also see an argument for keeping them separate.

def setup_app(settings: configs.ServiceSettings):
    app.include_router(health.router)
    #TODO: add pets router
    if settings.collectionName == "breeds":
    ...
    db = MockDB(settings)
    print(f"COLLECTION NAME {settings.collectionName}")
    @app.middleware("http")
...
def start(settings: configs.ServiceSettings):
    setup_app(settings)
    if settings.collectionName == "breeds":
        config = uvicorn.Config("be_python_assessment.services.service_launcher:app", host="0.0.0.0", port=settings.port, reload=True)
    else:
        config = uvicorn.Config("be_python_assessment.services.service_launcher:app", host="0.0.0.0", port=settings.port, reload=True) 
